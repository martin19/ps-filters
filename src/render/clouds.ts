...
export const filterDescriptorRenderClouds:FilterDescriptor = {
    id : "renderClouds",
    name : "Clouds",
    filter1 : filterRenderClouds1,
    filter4 : filterRenderClouds4
}
...
export interface FilterRenderCloudsOptions {
  foregroundColor : number[];
  backgroundColor : number[];
}

async function filterRenderClouds1(input:FilterInput, output:FilterOutput) {
  const i8 = new Uint8Array(input.img);
  const o8 = new Uint8Array(output.img);
  const w = input.w;
  const h = input.h;

  filterRenderCloudsCore1(i8, o8, w, h, {
    foregroundColor : input.foregroundColor ?? [255, 255, 255],
    backgroundColor : input.backgroundColor ?? [  0,   0,   0]
  });
}

async function filterRenderClouds4(input:FilterInput, output:FilterOutput) {
  const i8 = new Uint8Array(input.img);
  const o8 = new Uint8Array(output.img);
  const w = input.w;
  const h = input.h;
  ...
  filterRenderCloudsCore4([rI, gI, bI, aI], [rO, gO, bO, aO], w, h, {
    foregroundColor : input.foregroundColor ?? [255, 255, 255],
    backgroundColor : input.backgroundColor ?? [  0,   0,   0]
  });
...
function filterRenderCloudsCore1(input:U8Array, output:U8Array, w:number, h:number, options:FilterRenderCloudsOptions) {
  randomizePerlin();
  ...
  for(let y = 0; y < h; y++) {
    for(let x = 0; x < w; x++) {
      ...
      output[ofs  ] = sourceOver(255, output[ofs  ], val);
      ofs+=1;
    }
  }
}

function filterRenderCloudsCore4(input: U8Array[], output: U8Array[], w: number, h: number, options: FilterRenderCloudsOptions) {
  const i0 = input[0];
  const i1 = input[1];
  const i2 = input[2];
  const i3 = input[3];
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const val = scale * fbm(x / stepx, y / stepy, z, octaves) + offset;
      // const val = perlinNoise(x/stepx, y/stepy, options.z);
      o0[ofs] = sourceOver(fg[0], o0[ofs], val);
      o1[ofs] = sourceOver(fg[1], o1[ofs], val);
      o2[ofs] = sourceOver(fg[2], o2[ofs], val);
      o3[ofs] = 255;
      ofs++;
    }
  }
